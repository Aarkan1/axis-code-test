...
export const login = (username: string, password: string) =>
  fetchGraphQL<{ login: AuthPayload }>(loginMutation, {
    username,
    password,
  }).then((data) => data.login);

export const fetchCurrentUserCameras = (token: string) =>
  fetchGraphQL<CamerasResponse>(camerasQuery, undefined, token);

export const fetchAdminData = (token: string) =>
  fetchGraphQL<AdminDataResponse>(adminDataQuery, undefined, token);

export const addCamera = (
  token: string,
  input: Omit<Camera, "id"> & { userId: string },
) =>
  fetchGraphQL<{ addCamera: Camera }>(addCameraMutation, input, token).then(
    (data) => data.addCamera,
  );

export const addCameraToUser = (
  token: string,
  userId: string,
  cameraId: string,
) => fetchGraphQL(addCameraToUserMutation, { userId, cameraId }, token);

export const removeCameraFromUser = (
  token: string,
  userId: string,
  cameraId: string,
